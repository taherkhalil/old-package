...
class Packages(Document):
	def validate(self):
		...
		frappe.errprint(i_d)
		#If package doesn't already exists
		if not package in i_d:
			frappe.errprint("creating new package in items") 
			it = frappe.new_doc("Item")
			it.item_code = package
			it.item_name = package
			it.item_group = "Packages"
			it.is_stock_item =0
			it.category = "Package"
			it.sub_category = "service packages"
			it.description = package
			# it.standard_rate = cost
			it.insert(ignore_permissions=True)
			it.save()
			frappe.errprint("creation successfull")
			price_list = frappe.new_doc("Item Price")
			price_list.price_list = "Standard Selling"
			price_list.item_code = package
			price_list.price_list_rate =cost
...
# 			self.doc.outstanding_amount = flt(total_amount_to_pay - flt(paid_amount), self.doc.precision("outstanding_amount"))
